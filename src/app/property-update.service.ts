...
export class PropertyUpdateService {
  ...
  createProperty(payload: PropertyUpdatePayload): Observable<Property> {
    console.log('PropertyUpdateService: Creating new property with payload:', payload);
    // Assuming backend returns the saved Property object
    return this.http.post<Property>(`${this.baseUrl}/save`, payload).pipe(
      tap(response => console.log('PropertyUpdateService: Property creation successful:', response)),
      catchError(this.handleError)
    );
  }
